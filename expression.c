...
double gaussian( gsl_rng *generator, double mean, double sigma){
  double noise = 0;
  
  double r = gsl_ran_gaussian( generator, sigma);
...
char *argv[];
{
  ...
  double mutationEffect = 0., *expressionLevel;
  ...
  expressionLevel = (double *)calloc( nsam, sizeof(double));
  ...
  while( howmany-count++ ) {
    ...
    for( i = 0; i< segsites; i++){
      ...
      mutationEffect = gaussian(generator, step*1., 2.0);

      for( ind = 0; ind < nsam; ind++)
	if( list[ind][i] == '1' ){
	  nrepeats[ind] += step ;
	  expressionLevel[ind] += mutationEffect + gaussian( generator, 0, 1.);
	}
    }
    ...
    for( ind=0; ind < nsam-1; ind++) 
      printf("%f\t",expressionLevel[ind] );
    printf("%f",expressionLevel[nsam-1]);
    printf("\t%s",line+2 );
